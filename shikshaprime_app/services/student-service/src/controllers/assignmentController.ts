...
export async function getStudentAssignmentsAndStats(req, res, next: NextFunction) {
  try {
    ...
    const statsQuery = ` SELECT COUNT(*) AS total_assignments, 
        ...
        SUM(CASE WHEN asub.id IS NULL AND ta.due_date < CURDATE() THEN 1 ELSE 0 END) AS overdue_assignments 
        FROM teacher_assignments ta 
        INNER JOIN students s ON ta.program_id = s.program_id 
        AND ta.class_id = s.class_id AND ta.semester_id = s.semester_id 
        AND (ta.section_id = s.section_id OR ta.section_id IS NULL)
        LEFT JOIN student_assignment_submissions asub ON ta.id = asub.teacher_assignment_id 
        AND asub.student_id = s.id 
        AND asub.status IN ('submitted', 'graded') WHERE s.id = :studentId;`;
...
export async function getFilteredAssignments(req, res, next: NextFunction) {
  try {
    ...
    let query = ` 
    SELECT ta.id, ta.title, sub.name AS subject_name, ta.type, 
    ta.due_date,  (ta.allow_late_submissions = 1) AS allow_late_submissions, asub.id as assignment_submission_id, 
    CASE WHEN asub.id IS NOT NULL THEN 'Submitted' 
    WHEN asub.id IS NULL AND ta.due_date < CURDATE() THEN 'Overdue' 
    ...
    // Add ordering by due_date ascending
    ...
    return res.status(200).json({
      status: "success", message: "Filtered assignments fetched successfully",
      data: assignments,
    });
  } catch (error) {
    next(error);
...
export async function submitAssignment(req, res, next: NextFunction) {
  try {
    ...
    const upsertQuery = `INSERT INTO student_assignment_submissions 
        ...
        VALUES (:teacherAssignmentId, :studentId, :submissionText, :fileUrl, NOW(), NULL, NULL, NULL, NULL, NULL, 'not_submitted', NOW(), NOW())
    ...
    const query = `
      SELECT 
        ...
        GROUP BY asub.id, asub.teacher_assignment_id, ta.title, ta.description, ta.due_date,
                asub.submission_text, asub.file_url, asub.submitted_at, asub.status, asub.is_late_submission 
