...
-- Class broadcast queries
ALTER TABLE conversations 
ADD INDEX idx_class_broadcasts (type, class_id, is_active, created_at);

-- Message search and pagination
ALTER TABLE messages 
ADD INDEX idx_conversation_pagination (conversation_id, is_deleted, created_at DESC);

-- Teacher-student conversation lookup
ALTER TABLE conversation_participants 
ADD INDEX idx_direct_conversations (user_id, user_type, conversation_id);

COMMENT ON TABLE conversations IS 'Chat conversations - supports direct (teacher-teacher, teacher-student) and class broadcasts (teacher-to-class)';
COMMENT ON TABLE conversation_participants IS 'Who participates in each conversation with read tracking';
COMMENT ON TABLE messages IS 'All chat messages with threading and file support';
COMMENT ON TABLE message_read_status IS 'Individual message read receipts';
COMMENT ON TABLE class_broadcast_recipients IS 'Students who should receive class broadcast messages';
COMMENT ON TABLE chat_settings IS 'User chat preferences and online status';