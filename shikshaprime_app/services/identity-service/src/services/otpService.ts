...
export class OtpService {
          async sendOtp(phoneNumber: string, tenant:string) {
            const sequelize = getTenantSequelize(tenant);
                    // Check for recent OTP (prevent spam) — raw SQL used due to date arithmetic in WHERE
       ...
       ORDER BY created_at DESC LIMIT 1`,
                    ...
                    // Insert new OTP
                    await sequelize.query(
                              `INSERT INTO otp_requests (phone_number, otp_code, expires_at, attempts, created_at, updated_at)
       VALUES (:phoneNumber, :otp, :expiresAt, 0, NOW(), NOW())`,
       ...
       ON DUPLICATE KEY UPDATE verified_at = NOW(), updated_at = NOW()`,
          ...
          async sendEmailOtp(email: string, tenant: string) {
                    ...
                    // Insert new OTP
                    await sequelize.query(
                              `INSERT INTO email_otp_requests (email, otp_code, expires_at, attempts, created_at, updated_at)
                               VALUES (:email, :otp, :expiresAt, 0, NOW(), NOW())`,
