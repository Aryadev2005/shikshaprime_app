...
app.use("/api/finance/api-docs", swaggerUi.serve, swaggerUi.setup(swaggerDocument));
app.get("/api/finance/api-docs.json", (req, res) => {
...
const startServer = async () => {
  try {
    ...
    const server = app.listen(config.port, () => {
      console.log(`Finance service running on port ${config.port} in ${process.env.NODE_ENV || config.env}`);
    });
    // graceful shutdown
