...
import axios from "axios";
import { config } from "../config";
import { buildTenantFrontendUrl } from "./tenantUrlBuilder";
...
export class TSPHeadersUtil {
   ...
   */
  static async generateStandardHeaders(tenant: string, 
    ...
    try {
      merchantDomain = new URL(buildTenantFrontendUrl(tenant, "")).hostname;
    } catch {
      merchantDomain = buildTenantFrontendUrl(tenant, "") || "localhost";
    }
