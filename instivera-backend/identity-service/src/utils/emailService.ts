...
export const sendRegistrationEmail = async (
  ...
  try {
    await transporter.sendMail({
      from: '"Instivera" <noreply@instivera.com>',
...
export const sendOtpEmail = async (to: string, otp: string, name?: string): Promise<boolean> => {
  ...
  export const sendOtpEmailForVerification = async (to: string, otp: string): Promise<boolean> => {
    try {
      await transporter.sendMail({
        from: '"Instivera Team" <noreply@instivera.com>',
        to,
        subject: "Email verification OTP - Instivera",
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
