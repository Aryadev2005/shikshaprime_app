...
app.use("/api/teacher/api-docs", swaggerUi.serve, swaggerUi.setup(swaggerDocument));
app.get("/api/teacher/api-docs.json", (req, res) => {
...
app.use("/api/teacher/staff-attendance", staffAttendanceRoutes);
...
const startServer = async () => {
  try {
    ...
    const server = app.listen(config.port, () => {
      console.log(`Teacher service running on port ${config.port} in ${process.env.NODE_ENV || config.env}`);
    });
    // graceful shutdown
