...
export class AssignmentService {
        ...
        try {
            const query = `
                INSERT INTO teacher_assignments (
                    teacher_id, program_id, class_id, semester_id, academic_year_id,
    ...
    async getSubmittedAssignments(
        ...
        try {
            ...
            // ✔ department_id now comes from programs → departments
            if (department_id) {
                whereParts.push(
            ...
            // ✔ program_id now comes from student_personal_details
            if (program_id) {
                whereParts.push(
                    hasNumericProgramId
            ...
            // ✔ class_id now comes from student_personal_details
            if (class_id) {
                whereParts.push(
                    hasNumericClassId
                        ? "spd.class_id = :class_id"
                        : "(LOWER(c.name) LIKE LOWER(:class_name) OR LOWER(c.code) LIKE LOWER(:class_name))"
                );
            }

            if (roll_number) whereParts.push("st.roll_number LIKE :roll_number");
            if (student_id) whereParts.push("st.student_id LIKE :student_id");

            // ✔ student_name must be concatenated
            if (student_name) {
                whereParts.push(`
                TRIM(CONCAT(
                st.first_name, ' ',
            ...
            if (department_id) {
                replacements[hasNumericDepartmentId ? "department_id" : "department_name"] =
                    hasNumericDepartmentId ? department_id : `%${String(department_id).trim()}%`;
            }

            if (program_id) {
                replacements[hasNumericProgramId ? "program_id" : "program_name"] =
                    hasNumericProgramId ? program_id : `%${String(program_id).trim()}%`;
            }

            if (class_id) {
                replacements[hasNumericClassId ? "class_id" : "class_name"] =
                    hasNumericClassId ? class_id : `%${String(class_id).trim()}%`;
            }
            ...
            const [assignments, countResult] = await Promise.all([
                sequelize.query(assignmentsQuery, { replacements, type: QueryTypes.SELECT }),
    ...
    async gradeSubmission(
        ...
        try {
            ...
            try {
                // Find student user_id and assignment title
                const studentRes: any[] = await sequelize.query(`
                    SELECT COALESCE(s.user_id, u.user_id) AS user_id, ta.title AS assignment_title 
                    FROM students s
                    LEFT JOIN users u ON u.email COLLATE utf8mb4_general_ci = s.email COLLATE utf8mb4_general_ci
                    JOIN student_assignment_submissions sas ON sas.student_id = s.id
                    JOIN teacher_assignments ta ON ta.id = sas.teacher_assignment_id
                    WHERE sas.id = :submissionId
                `, {
                    replacements: { submissionId },
                    type: QueryTypes.SELECT
                });

                if (studentRes && studentRes.length > 0 && studentRes[0].user_id) {
                    const studentUserId = studentRes[0].user_id;
                    const assignmentTitle = studentRes[0].assignment_title || 'Assignment';

                    await sequelize.query(`
                        INSERT INTO notifications (user_id, title, message, type, channel, link, is_read, created_at, updated_at)
                        VALUES (:userId, 'Assignment Graded', :message, 'info', 'IN_APP', :link, 0, NOW(), NOW())
                    `, {
                        replacements: {
                            userId: studentUserId,
                            message: `Your assignment '${assignmentTitle}' has been graded.`,
                            link: `/student/student-assignment/view/${submissionId}`
                        },
                        type: QueryTypes.INSERT
                    });
                }
            } catch (notifErr) {
                console.error("[ASSIGNMENT GRADING] Failed to notify student:", notifErr);
            }
