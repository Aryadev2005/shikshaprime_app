...
app.use("/api/communication/api-docs", swaggerUi.serve, swaggerUi.setup(swaggerDocument));
app.get("/api/communication/api-docs.json", (req, res) => {
...
const startServer = async () => {
  try {
    ...
    const server = app.listen(config.port, () => {
      console.log(`Communication service running on port ${config.port} in ${process.env.NODE_ENV || config.env}`);
    });
    // graceful shutdown
