...
import { testConnection, getTenantModels } from "./models";
...
app.use(
  "/api/inventory/uploads/files",
  express.static(persistentUploadPath)
);

app.use("/api/inventory/api-docs", swaggerUi.serve, swaggerUi.setup(swaggerDocument));
app.get("/api/inventory/api-docs.json", (req, res) => {
...
app.use("/api/inventory", adminDashboardRoutes);
...
const startServer = async () => {
  try {
    ...
    const server = app.listen(config.port, () => {
      console.log(`Inventory management service running on port ${config.port} in ${process.env.NODE_ENV || config.env}`);
    });
    // graceful shutdown
