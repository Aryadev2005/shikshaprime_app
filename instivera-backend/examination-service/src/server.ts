...
app.use("/api/examination/api-docs", swaggerUi.serve, swaggerUi.setup(swaggerDocument));
app.get("/api/examination/api-docs.json", (req, res) => {
...
app.get("/", (req, res) => {
      res.json({
            status: "success",
            message: "Examination Service is running",
            data: {
...
app.use('/api/examination', routes);
...
const startServer = async () => {
  try {
    ...
    const server = app.listen(config.port, () => {
      console.log(`Examination service running on port ${config.port} in ${process.env.NODE_ENV || config.env}`);
    });
    // graceful shutdown
