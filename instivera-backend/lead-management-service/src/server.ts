...
import fs from "fs";
import router from "./routes/lead.routes";
...
app.use("/api/lead-management/api-docs", swaggerUi.serve, swaggerUi.setup(swaggerDocument));
app.get("/api/lead-management/api-docs.json", (req, res) => {
  res.json(swaggerDocument);
});

app.use("/api/lead-management/webhooks", webhookrouter);

app.use(tenantMiddleware);

app.use("/api/lead-management", router);
...
app.get("/", (req, res) => {
      res.json({
            status: "success",
            message: "Lead management Service is running",
            data: {
...
const startServer = async () => {
  try {
    ...
    const server = app.listen(config.port, () => {
      console.log(`Lead management service running on port ${config.port} in ${process.env.NODE_ENV || config.env}`);
    });
    // graceful shutdown
