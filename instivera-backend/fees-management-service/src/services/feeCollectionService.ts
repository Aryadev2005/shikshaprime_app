...
export class FeeCollectionService {
  ...
  async collectFee(payload, token, tenant): Promise<any> {
    ...
    return await sequelize.transaction(async (t) => {
      ...
      const studentPayments = fee_heads.map((h) => ({
        student_id,
        fee_head_id: h.fee_head_id,
        payment_type_id: paymentTypes.find(pyType => pyType.fee_head_id === h.fee_head_id).id,
        receipt_id: receipt.id,
        voucher_id: voucher.voucher_id,
        amount: h.amount,
        paid_amount: h.amount,
        paid_date: new Date(),
        status: "paid"        
      }));

      await models.StudentPayment.bulkCreate(studentPayments, { transaction: t });
      // -----------------------------------------
