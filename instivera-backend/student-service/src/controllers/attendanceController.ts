...
export const uploadAttendanceFile = async (req, res: Response) => {
  try {
    const file = req.file;
    const { uploaded_by, session_id, class_id } = req.body;
...
export const getStudentAttendanceSummary = async (req, res: Response) => {
  try {
    const { startDate, endDate, classId, programId, academicYearId, routineEntryId } = req.query;

    console.log(req.query);

    /*
    // Older code (caused EagerLoadingError: Department is not associated to Student!):
    ...
    */
    ...
    // Session attendance is the authoritative source for teacher subject stats.
    // The routine entry encodes the subject, class and assigned teacher, so the
    // client can never use a subject name alone to see unrelated students.
    if (routineEntryId) {
      const userId = req.user?.id || req.user?.user_id || req.user?.sub || 0;
      const email = req.user?.email || "";
      const username = req.user?.username || req.user?.employee_id || "";
      const [routineEntry]: any[] = await sequelize.query(
        `SELECT cre.id, cre.subject_id, cr.class_id
         ...
         WHERE cre.id = :routineEntryId
           AND (t.user_id = :userId
             OR (t.email IS NOT NULL AND t.email = :email AND :email != '')
             OR (t.employee_id IS NOT NULL AND t.employee_id = :username AND :username != ''))
         LIMIT 1`,
        { replacements: { routineEntryId: Number(routineEntryId), userId, email, username }, type: QueryTypes.SELECT }
      );

      if (!routineEntry) {
        return res.status(403).json({ status: 0, data: [], message: "You are not authorized to view this subject's attendance." });
      }

      let queryStart = startDate as string | undefined;
      let queryEnd = endDate as string | undefined;
      if (!queryStart || !queryEnd) {
        const now = new Date();
        const month = String(now.getMonth() + 1).padStart(2, "0");
        queryStart = `${now.getFullYear()}-${month}-01`;
        queryEnd = `${now.getFullYear()}-${month}-${String(new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate()).padStart(2, "0")}`;
      }
      ...
      const students: any[] = await sequelize.query(
        `SELECT st.id, st.student_id,
                ...
                st.roll_number, d.name AS department_name
         FROM students st
         LEFT JOIN student_personal_details spd ON spd.student_id = st.id
         LEFT JOIN programs p ON p.id = spd.program_id
         LEFT JOIN departments d ON d.id = p.department_id
         WHERE spd.class_id = :classId
            OR st.semester_id IN (SELECT semester_id FROM classes WHERE id = :classId)
         ORDER BY st.roll_number ASC, st.first_name ASC`,
        { replacements: { classId: routineEntry.class_id }, type: QueryTypes.SELECT }
      );

      const records: any[] = await sequelize.query(
        `SELECT sa.student_id, sa.attendance_status, sessions.attendance_date
         FROM student_attendance sa
         JOIN attendance_sessions sessions ON sessions.id = sa.attendance_session_id
         WHERE sessions.routine_entry_id = :routineEntryId
           AND sessions.status = 'SUBMITTED'
           AND sessions.attendance_date BETWEEN :queryStart AND :queryEnd`,
        { replacements: { routineEntryId: Number(routineEntryId), queryStart, queryEnd }, type: QueryTypes.SELECT }
      );
      const [sessionCountRow]: any[] = await sequelize.query(
        `SELECT COUNT(*) AS total FROM attendance_sessions
         WHERE routine_entry_id = :routineEntryId AND status = 'SUBMITTED'
           AND attendance_date BETWEEN :queryStart AND :queryEnd`,
        { replacements: { routineEntryId: Number(routineEntryId), queryStart, queryEnd }, type: QueryTypes.SELECT }
      );
      const totalSessions = Number(sessionCountRow?.total || 0);

      const summary = students.map((student: any) => {
        const studentRecords = records.filter((record) => String(record.student_id) === String(student.id));
        const presentDays = studentRecords.filter((record) => record.attendance_status === "PRESENT").length;
        const absentDays = totalSessions - presentDays;
        return {
          student_id: String(student.id), student_code: student.student_id,
          student_name: student.student_name, roll_number: student.roll_number || "N/A",
          dept_name: student.department_name || "N/A", present_days: presentDays,
          absent_days: absentDays, total_days: totalSessions,
          attendance_percentage: totalSessions ? Math.round((presentDays / totalSessions) * 100) : 0,
        };
      });
      return res.status(200).json({ status: 1, data: summary, message: "Subject attendance summary fetched successfully" });
    }
    ...
    if (classId) {
      whereConditions.push("spd.class_id = :classId");
      replacements.classId = classId;
    }
    if (programId) {
      whereConditions.push("spd.program_id = :programId");
      replacements.programId = programId;
    }
    if (academicYearId) {
      whereConditions.push("spd.academic_year_id = :academicYearId");
